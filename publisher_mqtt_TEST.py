...
def publish_mqtt_message(topic, payload, client):
    try:
...
def video_process(args, client):
    ...
    while video.isOpened():
        ...
        if current_frame % frames_to_skip == 0:
            ...
            publish_mqtt_message(args.topic, payload, client)
...
def main(args):
    ...
    mqtt_client = initialize_mqtt_client(args.device_id)

    if input_format == "video":
        video_process(args, mqtt_client)
    else:
        image_process(args, mqtt_client)
